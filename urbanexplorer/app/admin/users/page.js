              ...
              {users.map(user => (
                <tr key={user._id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                    <button onClick={() => handleChangeRole(user._id, user.role)} className={`${getRoleButtonColor(user.role)} text-white px-3 py-1 rounded-md transition-colors`}>
                      Zmień rolę
                    </button>
                    <button onClick={() => handleDeleteUser(user._id)} className="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded-md transition-colors">
                      Usuń użytkownika
                    </button>
                  </td>
                </tr>
